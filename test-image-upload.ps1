# Test image upload to local backend
$BackendUrl = "http://localhost:5000/api"
$TestEmail = "test@example.com"

Write-Host "🧪 Testing Image Upload to Local Backend" -ForegroundColor Cyan

# Create a minimal JPEG file (1x1 white pixel)
$jpegBytes = @(
    
)

[System.IO.File]::WriteAllBytes("$PSScriptRoot\test-profile-image.jpg", $jpegBytes)
Write-Host "✓ Created test image: test-profile-image.jpg" -ForegroundColor Green

# Prepare multipart form data
$uri = "$BackendUrl/auth/update-profile"
$filePath = "$PSScriptRoot\test-profile-image.jpg"

Write-Host "📤 Uploading image to: $uri" -ForegroundColor Yellow

try {
    $form = @{
        profileImage = Get-Item -Path $filePath
        currentEmail = $TestEmail
        name = "Test User"
        phone = "1234567890"
    }
    
    Write-Host "Form data:" -ForegroundColor Cyan
    foreach ($key in $form.Keys) {
        Write-Host "  $key = $($form[$key])" -ForegroundColor Gray
    }
    
    $response = Invoke-WebRequest -Uri $uri `
        -Method Post `
        -Form $form `
        -TimeoutSec 30
    
    Write-Host "`n✅ Upload successful!" -ForegroundColor Green
    Write-Host "`nResponse:" -ForegroundColor Cyan
    $response.Content | ConvertFrom-Json | ConvertTo-Json | Write-Host
    
} catch {
    Write-Host "`n❌ Upload failed!" -ForegroundColor Red
    Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Yellow
    
    if ($_.Exception.Response) {
        Write-Host "`nResponse content:" -ForegroundColor Yellow
        try {
            $reader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
            $reader.BaseStream.Position = 0
            $reader.ReadToEnd() | Write-Host
            $reader.Close()
        } catch {
            Write-Host "Could not read error response"
        }
    }
}
